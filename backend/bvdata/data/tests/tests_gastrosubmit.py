...
class TestGastroSubmit(TestCase):
    ...
    def test_gastrosubmit_form_is_valid(self) -> None:
        form = GastroSubmitForm(
            data=get_form_data(factory=GastroFactory, form=GastroSubmitForm)
        )
        self.assertTrue(form.is_valid())
        form.save()
        self.assertEqual(len(Gastro.objects.all()), 1)
    ...
    def test_gastrosubmit_view(self) -> None:
        client = Client()
        data = create_gastro_form_data(factory=GastroFactory, form=GastroSubmitForm)
