...
from bvdata.data.filters import BaseLocationFilter, GastroFilter
from bvdata.data.models import BaseLocation, Gastro
from bvdata.data.tests.factories import BaseLocationFactory, GastroFactory


class TestGastroFilter(TestCase):
    def test_is_closed_filter_legacy_gastro(self) -> None:
    ...
    def test_is_not_closed_filter_legacy_gastro(self) -> None:
        GastroFactory(closed=None)
        GastroFactory(closed=dt.now())
        qs = Gastro.objects.all()
