...
class TestViewSets(TestCase):
    ...
    def test_shopping_viewset_public_serializer_class(self) -> None:
        request = factory.get("/")
        request.user = AnonymousUser()
        gastro_view = ShoppingViewSet(request=request)
        serializer_class = gastro_view.get_serializer_class()
        self.assertIs(PublicShoppingDetailSerializer, serializer_class)

    def test_gastro_viewset_public_serializer_class(self) -> None:
        request = factory.get("/")
        request.user = AnonymousUser()
        gastro_view = GastroViewSet(request=request)
        serializer_class = gastro_view.get_serializer_class()
        self.assertIs(PublicGastroDetailSerializer, serializer_class)
    ...
    def test_gastro_view_public_no_private_fields(self) -> None:
        client = APIClient()
        location = BaseLocationFactory(
            type=LocationTypeChoices.GASTRO, is_submission=False, closed=None
        )
        ...
        response_get_list = client.get(
            reverse("data:api-v2-gastro-detail", args=[location.id_string])
        )
        location_data = response_get_list.json()
        for field in private_fields:
            self.assertTrue(field not in location_data)
